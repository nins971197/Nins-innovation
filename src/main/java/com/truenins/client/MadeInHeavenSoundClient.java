...
public final class MadeInHeavenSoundClient {
    ...
    private MadeInHeavenSoundClient() {}
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        boolean want = mc.player != null && mc.level != null
            && mc.player.hasEffect(TNMobEffects.MADE_IN_HEAVEN.get());

        if (want) {
            if (active == null || !mc.getSoundManager().isActive(active)) {
                active = new TNLoopingSound(SOUND, 0.8F);
